            ...
            <div class="row g-0">
                ...
                <div class="col-md-4 p-4 text-center border-end bg-light">
                    <div class="d-flex flex-column align-items-center h-100">
                        ...
                        <div class="avatar-wrapper mx-auto mb-3 position-relative">
                            <a href="#" data-bs-toggle="modal" data-bs-target="#avatarModal" class="avatar-link">
                                @if ($user->hasMedia('profile_pic'))
                                    <img src="{{ $user->getFirstMediaUrl('profile_pic') }}" class="avatar img-fluid rounded-circle shadow-sm" alt="User Avatar">
                        ...
                        <div class="mt-auto w-100">
                            ...
                            @else
                                <span class="badge bg-danger bg-opacity-10 text-danger px-3 py-2 rounded-pill fw-normal">
                ...
                <div class="col-md-8 p-4">
                    ...
                    <div class="d-flex flex-wrap justify-content-end gap-2 mt-4 pt-3 border-top">
                        @can('users_update')
                            <a href="{{ route('users.edit', $user->id) }}" class="btn btn-outline-primary px-4 rounded-pill">
                        ...
                        @can('users_delete')
                            <form action="{{ route('users.delete', $user->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-outline-danger px-4 rounded-pill" onclick="return confirm('{{ __('trans.delete_confirm') }}')">
            ...
            <div class="modal-body text-center py-4">
                <div class="mb-4">
                    <div class="avatar-preview mx-auto" style="width: 180px; height: 180px;">
                        @if ($user->hasMedia('profile_pic'))
                            <img src="{{ $user->getFirstMediaUrl('profile_pic') }}" class="img-fluid rounded-circle shadow" alt="User Avatar">
                        @else
                ...
                @can('users_update')
                <form action="{{ route('users.pic_upload', $user->id) }}" method="POST" enctype="multipart/form-data" class="px-3">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <input type="file" name="image" id="avatarUpload" accept="image/*" class="form-control d-none" required>
