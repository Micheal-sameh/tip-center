        ...
        <tbody>
            @foreach ($reports as $report)
                <tr
                    ...
                    @if ($reports->contains(fn($r) => $r->materials > 0))
                        <td class="text-center">{{ $report->materials ?? 'N/A' }}</td>
                    @endif
                    @if ($reports->contains(fn($r) => $r->printables > 0))
                        <td class="text-center">{{ $report->printables ?? 'N/A' }}</td>
                    @endif
                    <td class="text-center fw-bold">
                        {{ number_format($report->professor_price + $report->center_price, 2) }}
                    </td>
                    @if ($reports->contains(fn($r) => $r->to_pay + $r->to_pay_center + $r->to_pay_print + $r->to_pay_materials > 0))
                        <td class="text-center">{{ $report->to_pay + $report->to_pay_center + $report->to_pay_print + $report->to_pay_materials ?? 'N/A' }}</td>
                    @endif
                </tr>
            @endforeach
            @if (count($reports) > 0)
